...
- Keyboard focus movement could not be independently confirmed with the in-app browser's control layer; existing automated keyboard coverage remains the evidence boundary.
...
- Real iOS/Android device testing and a formal Lighthouse/WebPageTest run remain unverified (`SPR-UNVERIFIED`). Passing emulated viewport tests is not real-device evidence.
...
- Fresh observed route completion was about 4.1 seconds for the 737-row Compare view and 4.4 seconds for the focused Atlas. The repository's landing-performance contract passed in the live suite, but it does not make the Atlas interaction acceptable.
