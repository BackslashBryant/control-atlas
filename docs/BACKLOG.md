...
| LENS-JOB-001 | The Atlas "by job" lens places all 28 publications into six jobs, and that placement is authored rather than derived — no field in the corpus records what a reader is trying to do. Each grouping carries its reasoning in `data/curated/framework-lenses.json`. A practitioner should confirm the six jobs are the right six and that no publication is filed under the wrong one. | Product owner | Before the job lens is presented as authoritative guidance rather than a way in |
