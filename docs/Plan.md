...
| Publisher-field presentation | Met | Automated + live browser | Source dates are separated; source text blocks, exact retained locators, procedure/config formats, governed publication names in facts and breadcrumbs, and copy behavior pass focused regressions across the required widths and source families. Live compact and desktop checks confirm exact locator copy, safe wrapping, and no raw publication slug. |
