...
**Not whole-product complete.** Deployed main `c6e499e91afbb09433f3b5514d8b5aeb4ddcf315` passed exact-head and merged-main CI, Pages deployment, live smoke, matching `release.json`, and focused six-width production verification. Official-source review, exact source-locator presentation, governed publication labels/currentness, DISA CCI taxonomy coverage, Atlas local-filter containment, human-first generated record identity, and accessible human parent-source provenance are shipped and live-verified. The active milestone gives each Source detail route its specific source identity while retaining shared family context. Hands-on Android Chrome/TalkBack plus desktop NVDA evidence remains unavailable. The legacy `ashbryant.github.io` 404 is an external ownership limitation, not a product-code fix. No test, structural reconciliation, or emulated viewport is being promoted into those missing evidence classes.
...
| Publisher-field presentation | Partial | Automated + live browser | Source text blocks, exact retained locators, procedure/config formats, governed publication names, separate source-check/publication-review evidence, copy behavior, human-first generated record identity, and human parent-source inheritance explanations pass focused regressions. Source detail routes still reuse generic family labels as H1/browser identity (POLISH-025). |
...
| Six-width route/template quality | Met for the current route matrix | Automated + local rendered browser + live browser | The route matrix is clean for page overflow, navigation, empty states, bounded rendering, Atlas local-filter containment, and generated/native record identity at 320, 375, 390, 768, 1024, and 1440. Source-detail identity remains separately tracked as POLISH-025 until live verification. |
...
| Canonical live release | Met | CI + Pages + live browser | Release `c6e499e91afbb09433f3b5514d8b5aeb4ddcf315` passed exact-head and merged-main CI, fresh checkout, GitHub Pages, Pages Live Smoke, and the six-width live Sources inheritance regression. `release.json` matched the merged SHA. |
...
- Human parent-publication names in Sources inheritance explanations are shipped and live-verified. The active milestone gives Source detail routes specific identity (POLISH-025); removal of the duplicate return action (POLISH-026) and invalid-detail recovery (POLISH-027) remain next, followed by continued source-backed taxonomy review without converting missing tags into negative applicability.
...
2. **Product and accessibility evidence**
   - Close POLISH-025, POLISH-026, and POLISH-027 on Source detail identity/navigation/recovery, then restore a met publisher-presentation verdict after live verification.
