...
| Publisher-field presentation | Partial | Automated + live browser | Source text blocks, exact retained locators, procedure/config formats, governed publication names, separate source-check/publication-review evidence, copy behavior, and human-first generated record identity pass focused regressions. Sources inheritance explanations still announce raw parent source IDs to assistive technology (POLISH-024). |
...
| Canonical live release | Met | CI + Pages + live browser | Release `d632bdf044a95e964ec9b666e19f5f6e0a02af81` passed exact-head and merged-main CI, fresh checkout, GitHub Pages, Pages Live Smoke, and the six-width live generated/native identity regression. `release.json` matched the merged SHA. |
...
- The active milestone resolves human parent-publication names in Sources inheritance explanations (POLISH-024). The next isolated source-detail milestones are specific route identity (POLISH-025) and removal of the duplicate return action (POLISH-026), followed by continued source-backed taxonomy review without converting missing tags into negative applicability.
...
2. **Product and accessibility evidence**
   - Close POLISH-024 without exposing raw parent source IDs to assistive technology, then restore a met publisher-presentation verdict after live verification.
