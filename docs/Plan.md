...
- [x] **T9.1** Pin or vendor the current approved Orbital release and record the version.
- [x] **T9.2** Preserve token drift tests.
- [x] **T9.3** Map Control Atlas semantic tokens to Orbital tokens without inventing duplicate color systems.
- [x] **T9.4** Audit all used components against `components/contracts.json`, including all required states.
- [x] **T9.5** Use the Orbital icon manifest before adding or retaining a one-off icon.

#### Color and typography

- [x] **T9.6** Complete the decorative teal sweep:
...
- [x] **T9.7** Keep color token values centralized. Do not fix misuse by redefining the palette.
- [x] **T9.8** Use condensed display type for headings, readable body type for sustained text, and pixel/silkscreen accents only for brief micro-labels.
- [x] **T9.9** Remove hardcoded tracking values in favor of tokens.

#### Geometry and texture

- [x] **T9.10** Apply one dominant and at most one supporting texture per surface.
- [x] **T9.11** Keep operational pages calmer than the landing page.
- [x] **T9.12** Put calibration rails, ticks, datum marks, and vectors only in safe corridors and only when tied to actual content.
- [x] **T9.13** Remove faux coordinates, faux telemetry, meaningless arcs, and lines through interactive content.
- [x] **T9.14** Ensure decorative geometry is `aria-hidden` and pointer-inert.

#### CSS architecture

- [x] **T9.15** Incrementally retire page rules from `styles/surfaces.css`.
- [x] **T9.16** Consolidate shared component rules in the correct component layer.
- [x] **T9.17** Remove arbitrary Tailwind values and important overrides from component files when a token/component rule exists.
- [x] **T9.18** Remove Tailwind class overrides in `orbital.css`.
- [x] **T9.19** Preserve the known cascade rule: `orbital.css` imports last and wins ties; eliminate conflicting duplicate selectors rather than escalating specificity.
- [x] **T9.20** Track CSS line counts, duplicate declarations, unused selectors, and remaining compatibility rules after every page migration.
