...
| Publisher-field presentation | Partial | Automated + live browser | Source text blocks, exact retained locators, procedure/config formats, governed publication names, separate source-check/publication-review evidence, and copy behavior pass focused regressions. Generated Control Atlas record keys still displace publisher-authored identity in the deployed record and Library surfaces (POLISH-023). |
...
| Canonical live release | Met | CI + Pages + live browser | Release `0766e67abb2dd02ad3871a875273a521aa3d97f6` passed exact-head and merged-main CI, fresh checkout, GitHub Pages, Pages Live Smoke, and the six-width live source/currentness regression. `release.json` matched the merged SHA. |
...
- The active P1 is generated record identity: preserve canonical keys in routes and data while using publisher-authored titles, human record types, and governed publication names in headings, browse results, breadcrumbs, browser titles, and accessible names.
...
- Next product milestone after generated record identity: resolve human parent-publication names in Sources inheritance explanations (POLISH-024), then continue source-backed taxonomy review without converting missing tags into negative applicability.
...
2. **Product and accessibility evidence**
   - Fix and live-verify POLISH-023 before restoring a met publisher-presentation verdict; then close POLISH-024 without exposing raw parent source IDs to assistive technology.
