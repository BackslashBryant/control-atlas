...
- [x] **T8.1 Home** — use `landing-page.html` / editorial split:
  ...
  - **Addendum — creator-directed splash fixes (post-review):** three specific defects in the Depth-0 `signal-cover` itself, reported directly by the creator after the above pass, folded into T8.1 as Home/splash surface work:
    1. **Dismissal was too permissive.** `connectSignalCover()` dismissed on click-anywhere-on-the-cover, mouse wheel, touchmove, Space, and Escape, in addition to the intended Enter/button-click. Locked dismissal to exactly two triggers: an `Enter` keydown, or a click on a new dedicated `<button data-signal-cover-enter>` element (previously the whole cover was `role="button" tabindex="0"`, making the entire surface — not just the visible orange action — a click target). Updated the cover's own ARIA to `role="dialog" aria-modal="true" aria-labelledby="signal-cover-headline"`, focuses the button (not the whole cover) on reveal, and reworded the on-page prompt from "Click anywhere or press Enter to start" (now false) to "Press Enter or select Enter the Atlas to start."
    2. **Flash-of-Home-before-splash.** Root-caused via injected performance-timeline instrumentation (not guesswork): `.signal-cover` had `animation: signal-cover-in 460ms … ; @keyframes { from { opacity: 0 } to { opacity: 1 } }` on the *entire* fixed-position cover, including its opaque background layer. For the first portion of that 460ms entrance, the background itself was semi-transparent while `.home-entry` had already become visible underneath in the same instant (confirmed atomic via `MutationObserver`) — so the Home surface visibly showed through the cover during the fade-in. Moved the opacity animation off `.signal-cover` (background is now instantly, fully opaque the moment it's unhidden — verified `getComputedStyle(cover).opacity === '1'` on every sampled frame from first paint) and onto `.signal-cover__copy`/`.signal-cover__meta` instead (a `translateY`+opacity settle-in on the *content*, which never compromises coverage since the opaque background is unaffected). Reproducing this required disabling Playwright's default `navigator.webdriver = true` (which the cover's own logic uses to skip itself for e2e/automation) — real visitors, not automation, hit this path.
    3. **Fabricated-looking meta rail replaced with real KPIs.** The existing archival aside showed static, non-numeric labels ("Coverage: Federal-wide," "Publishers: NIST · DISA · FedRAMP," "Connections: Cross-mapped") — already arguably in tension with this task's own "no fake telemetry" bullet. Replaced with three real, build-time-computed counts in the same Orbital telemetry-readout grammar (existing `.signal-cover__meta` CSS — gold mono values, right-aligned — needed no styling change): **Publications** (47, `data/generated/publication-identity-index.json` `identity_count`, matches Sources' own canonical register exactly), **Crosswalks** (24, unique undirected catalog-pairs in `data/generated/catalog-bootstrap.json` `mapping_sources`, matching Compare's Phase 6 framework-to-framework scope), **Controls** (1,196, the `controls` row of `data/generated/connection-inventory.json`, matching the NIST SP 800-53 catalog's own displayed count). Computed in a new `computeSignalCoverKpis()` in `vite.config.ts` (reads the generated JSON directly via `readFileSync`, same build-time-only approach already used for `HOME_TAG_GROUPS`) — never hardcoded, so the numbers move automatically the next time `npm run build:data` regenerates these artifacts.
    - **Verified:** wrote a real-visitor behavioral test harness (`navigator.webdriver` spoofed to `false`, since the cover intentionally skips itself for automation) exercising all five previously-working dismiss triggers plus the two intended ones: click-on-backdrop, mouse-wheel, and Escape each correctly leave the cover in the DOM (not dismissed); Enter and a direct click on the button each correctly remove it. Timing instrumentation confirmed `cover-opacity` reads `1` on every animation frame from the first frame the cover exists — no transparent frame, no Home bleed-through. Built markup inspected directly: `Publications 47`, `Crosswalks 24`, `Controls 1,196` matched known-correct figures cross-checked against the Sources and Library pages. `npm run typecheck` — 0 errors. `npx eslint` on all touched files — 0 errors/warnings. `npm run test:copy-contract`, `node --test tests/content-review.test.mjs`, `npm run test:browser` — all green (no existing test referenced the old cover markup/copy, so none needed updating). `npx playwright test` across `epic14-ws0-app-shell`, `epic13-reference-workbench`, `epic14-ws5-home-guides` — 13 passed, 2 failed, both the same pre-existing `epic13-reference-workbench` failures already isolated earlier in this phase (unrelated, generated-data drift). `npm run test` (full unit composite) — 0 failures. `npm run build:site` — successful.
    - Wide and narrow screenshots captured of the live splash (`splash-wide.png`, `splash-narrow.png`) and reviewed by eye: the "At a glance" KPI panel renders cleanly at both widths with no overflow, the focused Enter button shows a visible focus ring, and the prompt copy is now accurate.
...
- [x] **T8.9 Global search and navigation** — use top-navigation reference and component contracts:
  - stable active state;
  - accessible overflow;
  - consistent search;
  - no route dead ends;
  - clear mobile labels.
  - **Audit finding:** the substantive gap here — Start Here and Documents unreachable from any nav surface, breaking "no route dead ends" — was the real defect found and fixed during T8.1's audit (`navigation.ts`, `routeIdentity.ts`, `src/index.html`'s static header) and re-verified there against clean `main`. T8.9 closes the loop with a dedicated pass over the remaining bullets using the live app rather than re-deriving them: the desktop overflow ("Open more pages") menu now lists Guides and Documents with icons in a properly Orbital-toned panel; the mobile sheet ("Open navigation menu") groups all nine destinations under "Explore Control Atlas" and "Guides and information" with icon + visible text label on every item (never icon-only); `aria-current`/active-tab styling is stable across Start Here, Atlas, Library, Compare, Resources, Sources, and About (covered by `tests/e2e/epic12-phase5-links-semantics-touch.spec.mjs`'s per-route aria-current loop, unaffected by Phase 8); and the search overlay renders with identical placeholder copy, focus ring, and backdrop treatment whether opened from Home's inline search trigger or the header's "Open search" button on an interior page like Atlas.
  - **Fix:** none beyond T8.1's nav/route-reachability fix — this task's remaining bullets were already satisfied by the existing `TopNav.tsx`/`SearchOverlay.tsx` implementation.
  - **Verified:** re-ran the full nav-relevant Playwright set post-T8.8 to confirm no drift across all of Phase 8's changes: `epic12-phase3-information-architecture.spec.mjs`, `epic14-ws0-app-shell.spec.mjs`, `navigation-fidelity.spec.mjs`, `taxonomy-discovery.spec.mjs` — 25 passed, 8 failed. Isolated the 5 unfamiliar failures (`navigation-fidelity.spec.mjs` legacy-alias/search-field/Atlas-bundle assertions, `taxonomy-discovery.spec.mjs` governed-tag handoff and STIG-chain-compare assertions) with a temporary `git worktree add ../control-atlas-main-check main` comparison build (removed after use, per the no-junk-worktree rule) rather than assuming: all reproduced identically against clean `main`, unrelated to any Phase 8 change and consistent with the same generated-data-drift pattern (`.compare-control-surface`/`.atlas-tree` locators, STIG-chain state) already isolated during T8.1, T8.3, and T8.5 verification. Zero regressions confirmed across all nine Phase 8 pages.
  - Screenshots captured of the desktop overflow menu, the full mobile navigation sheet, and the search overlay opened from a non-Home route, and reviewed by eye: all three render as intended dark Orbital panels with icons and visible labels, no unstyled or default-browser chrome.
...
6. **T8.1 signal-cover (creator-reported, post-review)** — the Depth-0 splash dismissed on click-anywhere/wheel/touchmove/Space/Escape instead of only Enter/button-click; its own entrance animation faded the *entire opaque background* from `opacity:0`, letting Home visibly show through mid-transition (root-caused with injected performance-timeline instrumentation, not guesswork); and its metadata aside showed static non-numeric labels rather than real numbers. Locked dismissal to exactly two triggers, moved the opacity animation off the background onto the inner content only, and replaced the aside with three build-time-computed KPIs (Publications, Crosswalks, Controls) read directly from generated data. See the T8.1 addendum above for full detail.
...
- **Re-verified after the T8.1 signal-cover addendum** (dismiss lock, no-flash timing, real KPIs): `npm run typecheck`, full `npm run lint`, `npm run test` (0 failures), `npm run test:copy-contract`, `node --test tests/content-review.test.mjs`, `npm run test:browser`, and `npx playwright test` across `epic14-ws0-app-shell`/`epic13-reference-workbench`/`epic14-ws5-home-guides` all re-run clean after the addendum's commits, identical results to the pre-addendum pass above.
...
- Chose Publications/Crosswalks/Controls as the three signal-cover KPIs (creator explicitly deferred metric selection: "use your judgment"). Considered a raw total-records count (30,190+) as a fourth or replacement stat but judged the three chosen numbers more legible and more directly tied to what a newcomer would want to know ("how much is here, how connected is it, how deep does it go") than one very large, less contextualized number. Labeled the aggregate "Controls" (matching `connection-inventory.json`'s own category label) rather than a framework-specific label, since the underlying count is a genuine platform-wide schema category, not fabricated or arbitrarily scoped to one publisher.
- Kept the Enter key listener global (not scoped to when the button itself has focus) so "press Enter to start" works immediately on reveal without requiring an extra Tab first — consistent with the button receiving focus automatically when the cover opens.
