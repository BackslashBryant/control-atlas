...
/** @type {[number, number]} */
const PDF_PAGE = [612, 792]; // US Letter in PDF points.
const PDF_MARGIN = 48;

function pdfSafeText(value) {
  return cleanInlineMarkdown(String(value ?? ""))
    .replace(/[\u2013\u2014]/g, "-")
    .replace(/[\u2018\u2019]/g, "'")
    .replace(/[\u201C\u201D]/g, '"')
    .replace(/\u2022/g, "-")
...
function pdfWrap(text, font, size, width) {
  const lines = [];
...
/**
 * Serialize the structured template document to a branded, print-ready PDF.
 * Tables are rendered as readable record cards so wide working registers do
 * not collapse into unreadable columns on Letter paper.
 *
 * @param {any} doc
...
export async function docToPdf(doc) {
  ...
  pdf.setTitle(pdfSafeText(doc.title || "Control Atlas starter document"));
  pdf.setAuthor("Control Atlas");
  pdf.setSubject("Control Atlas starter document");

  const regular = await pdf.embedFont(StandardFonts.Helvetica);
  const bold = await pdf.embedFont(StandardFonts.HelveticaBold);
  const navy = rgb(23 / 255, 54 / 255, 93 / 255);
  const teal = rgb(13 / 255, 111 / 255, 118 / 255);
  const slate = rgb(71 / 255, 84 / 255, 103 / 255);
  const pale = rgb(244 / 255, 246 / 255, 249 / 255);
  const border = rgb(214 / 255, 222 / 255, 232 / 255);
  let page = pdf.addPage(PDF_PAGE);
  let cursorY = 0;
  let hasStartedPage = false;

  const drawBrandHeader = (targetPage) => {
    targetPage.drawRectangle({ x: 0, y: 744, width: 612, height: 48, color: navy });
    targetPage.drawText("CONTROL ATLAS", {
      x: PDF_MARGIN,
      y: 763,
      font: bold,
      size: 10,
    ...
    targetPage.drawText("REFERENCE AID", {
      x: 444,
      y: 763,
      font: bold,
      size: 8,
  ...
  const beginPage = () => {
    if (hasStartedPage) page = pdf.addPage(PDF_PAGE);
    drawBrandHeader(page);
    cursorY = 722;
    hasStartedPage = true;
  };
  const ensureSpace = (height) => {
    if (cursorY - height < PDF_MARGIN + 18) beginPage();
  };
  const drawWrapped = (text, options = {}) => {
    ...
    const width = 612 - PDF_MARGIN * 2 - indent;
    const leading = options.leading || size * 1.42;
    const lines = pdfWrap(text, font, size, width);
  ...
  beginPage();
  drawWrapped(doc.title, { font: bold, size: 20, color: navy, leading: 27 });
  drawWrapped(doc.description, { size: 10, color: slate, leading: 15 });
  cursorY -= 6;

  const disclaimerLines = pdfWrap(`Important: ${DISCLAIMER}`, regular, 8.5, 492);
  const disclaimerHeight = disclaimerLines.length * 12 + 16;
  ensureSpace(disclaimerHeight);
  page.drawRectangle({
    x: PDF_MARGIN,
    y: cursorY - disclaimerHeight,
    width: 516,
  ...
  for (const section of doc.sections || []) {
    ensureSpace(30);
    cursorY -= 5;
    drawWrapped(section.heading, { font: bold, size: 13, color: navy, leading: 18 });
    if (section.type === "text") {
      drawWrapped(section.content, { size: 9.5, color: slate, leading: 14 });
      cursorY -= 5;
      continue;
    }
    ...
    const headers = section.headers || [];
    const rows = section.rows || [];
    drawWrapped(`${rows.length} ${rows.length === 1 ? "entry" : "entries"} - complete in your working copy.`, {
      size: 8.5,
      color: slate,
      leading: 12,
    });
    for (const [rowIndex, row] of rows.entries()) {
      const populated = headers
        .map((header, index) => [header, row?.[index] ?? ""])
        .filter(([, value]) => String(value).trim());
      const fields = populated.length > 0 ? populated : headers.map((header) => [header, ""]);
      const cardLines = fields.flatMap(([header, value]) => [
        ...pdfWrap(`${header}: ${value || "[Complete this field]"}`, regular, 8.5, 486),
      ]);
      const cardHeight = Math.max(34, cardLines.length * 12 + 18);
      ...
      page.drawText(`Entry ${rowIndex + 1}`, {
        x: PDF_MARGIN + 10,
        y: cursorY - 12,
        font: bold,
        size: 8.5,
  ...
  pages.forEach((currentPage, index) => {
    // Draw the brand band last so a very tall record card can never obscure it
    // at the top of a new page.
    drawBrandHeader(currentPage);
    currentPage.drawLine({
      start: { x: PDF_MARGIN, y: 30 },
      end: { x: 612 - PDF_MARGIN, y: 30 },
      thickness: 0.5,
      color: border,
    });
    currentPage.drawText("Control Atlas - starter document", {
      x: PDF_MARGIN,
      y: 18,
      font: regular,
      size: 7.5,
...
/**
 * Render a document to PDF bytes suitable for browser preview or download.
 * @param {any} doc
 * @returns {Promise<{ bytes: Uint8Array, mimeType: string, extension: string }>}
 */
export async function renderPdfDocument(doc) {
  return { bytes: await docToPdf(doc), mimeType: PDF_MIME, extension: "pdf" };
}
