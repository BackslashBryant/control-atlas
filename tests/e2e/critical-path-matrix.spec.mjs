...
test('critical path: landing hero and primary entry cards are visible', async ({ page }) => {
  await page.goto('/');
...
test('critical path: compare detailed mappings expose text provenance labels', async ({ page }) => {
...
test('critical path: library detail relationships show connection and source trust text', async ({ page }) => {
  ...
  await expect(relationshipCard.locator('.relationship-meta span').first()).not.toBeEmpty();
  await expect(relationshipCard.locator('.relationship-meta span').nth(1)).not.toBeEmpty();
});

test('critical path: STIG chain summary table is labeled for screen readers', async ({ page }) => {
  await page.goto('/?view=matrix&workbench=stig-chain');
  await waitForAppReady(page);
  await dismissOnboarding(page);

  await expect(page.getByRole('table', { name: 'STIG chain summary' })).toBeVisible();
  await expect(page.getByText('Official link = published mapping')).toBeVisible();
});

test('critical path: baseline compare surfaces delta controls with export actions', async ({ page }) => {
...
test('critical path: keyboard focus reaches primary nav and header search', async ({ page }) => {
  await page.goto('/');
  await waitForAppReady(page);
  await dismissOnboarding(page);

  const primaryNav = page.getByRole('navigation', { name: 'Primary navigation' });
  const startHere = primaryNav.getByRole('button', { name: 'Start Here', exact: true });
